...
function parseDomain(url, options) {
  ...
  var hostWithoutTld = hostname.slice(0, -tld.length).split(dot);
  ...
  var hostWithoutTldOrDomain = hostWithoutTld.pop();
  var subdomain = hostWithoutTld.join(".");
  return {
    tld: tld,
